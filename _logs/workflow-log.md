...
| 2026-05-27 | Source inventory + pilot 002/003 case cards | 0_raw/metadata / 1_digest / 2_framework | 0_raw/metadata/source-inventory.md; 0_raw/metadata/source-index.md; 1_digest/case_cards/CC-COURT-ZHANG-32-ILLEGAL-MINING-YANCHENG.md; 1_digest/case_cards/CC-COURT-CHEN-GUJK-CULTURAL-RELIC.md; 2_framework/pilot_checks/pilot-note-002-003.md | pilot/source-inventory-select-002-003 分支；扫描全部 0_raw 案例材料（20个文件），建立 source-inventory.md；选定两个对照型 pilot 案例，生成 case card；digest card validator 和 layer boundary validator 均通过；python mcp/smoke_test.py 通过（29/29），python mcp/smoke_v1.py 通过（9/9）；pilot 选定理由和对比维度记录在 2_framework/pilot_checks/pilot-note-002-003.md；更新 source-index.md；未进入 3_synthesis；仍需人工复核 case card 与原文一致性；未新增 MCP 功能、未修改 MCP v1 工具逻辑、未生成 final report。 |
| 2026-05-27 | Framework v1 lock | 2_framework | 2_framework/FRAMEWORK_V1_LOCK.md | framework/lock-v1-after-pilot 分支；基于 3 个 pilot case cards（SPP_TYPICAL_WUHAN_ILLEGAL_MINING、COURT_ZHANG_32_ILLEGAL_MINING_YANCHENG、COURT_CHEN_GUJK_CULTURAL_RELIC）锁定 framework v1；锁定内容：case card v1 字段（必填/建议）、case type 标签体系（source_level/proceeding_type/punitive_result/intentionality_path/calculation_structure/infringement_type）、comparison table v1 字段（P0 主列 13 项 + P1 辅助列 7 项）、synthesis claim rules v1（8 条基本规则 + 表述强度分级）；明确当前 3 个 pilot 均为"支持"类型，尚未覆盖"不支持"类型，该类型应在 batch 阶段优先补充；python mcp/smoke_test.py 通过（29/29），python mcp/smoke_v1.py 通过（9/9）；未生成 final report、未生成横向法律结论、未新增 MCP 功能、未修改 MCP v1 工具逻辑、未新增 case card。 |
